...
class UserRegisterAPITesting(APITestCase):
    ...
    def test_registration_failed_bad_request_incorrect_param(self):
        response=self.client.post(reverse('register'),{'username':'some_username','password':'qwerty123','email':'rjhhxgfhdfghgfhgf'})
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
    ...
    def test_registration_same_email(self):
        response=self.client.post(reverse('register'),{'username':'another_test','password':'test','email':'test@email.com'})
